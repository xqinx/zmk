...
struct thb_data {
    ...
    int16_t xy_raw[2]; // buffer for adc raw value read
#ifdef CONFIG_JOYSTICK_THB_TRIGGER
    sensor_trigger_handler_t trigger_handler;
    struct sensor_trigger trigger;
    int32_t trigger_fs; // sampling frequency as part of attributes
...
static int thb_sample_fetch(const struct device *dev, enum sensor_channel chan) {
    ...
    // First read is setup as calibration, after that should be normal read
    as->calibrate = false;
...
static int thb_trigger_set(const struct device *dev, const struct sensor_trigger *trig,
    ...
    // only support SENSOR_TRIG_DATA_READY type of trigger on SENSOR_CHAN_ALL
...
static int thb_attr_set(const struct device *dev, enum sensor_channel chan,
    ...
    // max sample rate limit to 100KHz
    if (val->val1 > 100000) {
        LOG_DBG("Sample rate should not exceed 100KHz");
        return -EINVAL;
    }

    // ignore the fraction part of val
...
static void thb_timer_cb(struct k_timer *item) {
    ...
    // all thb device share one workqueue
...
static int thb_init(const struct device *dev) {
    ...
    struct adc_channel_cfg channel_cfg = {
        // 600mV internal / (1/3) GAIN -> 1.8V range
        .gain = ADC_GAIN_1_3,
    ...
    drv_data->as = (struct adc_sequence){
        ...
        // Note oversampling is only supported for single channel only
        // see adc_nrfx_saadc.c
        .oversampling = 0,
        .resolution = 12,
        .calibrate = true,
    };
